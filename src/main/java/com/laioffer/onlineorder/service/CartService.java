...
public class CartService {
    ...
    public void updateItemQuantity(long customerId, long orderItemId, int quantity) {
        CartEntity cart = cartRepository.getByCustomerId(customerId);
        OrderItemEntity orderItem = findOwnedOrderItem(cart, orderItemId);
        ...
        if (quantity <= 0) {
            ...
            orderItemRepository.save(new OrderItemEntity(
                    orderItem.id(), orderItem.menuItemId(), orderItem.cartId(), orderItem.price(), quantity));
        }
        cartRepository.updateTotalPrice(cart.id(), orderItemRepository.sumPriceByCartId(cart.id()));
    }
    ...
    public void removeItem(long customerId, long orderItemId) {
        CartEntity cart = cartRepository.getByCustomerId(customerId);
        findOwnedOrderItem(cart, orderItemId);
    ...
    // 越权校验：order item 必须属于当前用户自己的购物车。
    // 对别人的 item 返回 404 而不是 403，避免通过状态码泄漏"这个 id 存在"。
    private OrderItemEntity findOwnedOrderItem(CartEntity cart, long orderItemId) {
        OrderItemEntity orderItem = orderItemRepository.findById(orderItemId).orElse(null);
        if (orderItem == null || !orderItem.cartId().equals(cart.id())) {
            throw new ResourceNotFoundException("Order item " + orderItemId + " not found in your cart");
        }
        return orderItem;
    }
