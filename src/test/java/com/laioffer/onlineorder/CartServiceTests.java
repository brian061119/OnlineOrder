...
public class CartServiceTests {
    ...
    @Test
    void updateItemQuantity_shouldSaveNewQuantityAndRecalculateTotal() {
        long customerId = 1L;
        long cartId = 3L;
        long orderItemId = 4L;
        CartEntity cartEntity = new CartEntity(cartId, customerId, 10.0);
        OrderItemEntity orderItem = new OrderItemEntity(orderItemId, 2L, cartId, 10.0, 1);

        Mockito.when(cartRepository.getByCustomerId(customerId)).thenReturn(cartEntity);
        Mockito.when(orderItemRepository.findById(orderItemId)).thenReturn(Optional.of(orderItem));
        Mockito.when(orderItemRepository.sumPriceByCartId(cartId)).thenReturn(30.0);

        cartService.updateItemQuantity(customerId, orderItemId, 3);

        Mockito.verify(orderItemRepository).save(new OrderItemEntity(orderItemId, 2L, cartId, 10.0, 3));
        Mockito.verify(cartRepository).updateTotalPrice(cartId, 30.0);
    }


    @Test
    void updateItemQuantity_whenQuantityIsZero_shouldDeleteTheItem() {
        long customerId = 1L;
        long cartId = 3L;
        long orderItemId = 4L;
        CartEntity cartEntity = new CartEntity(cartId, customerId, 10.0);
        ...
        Mockito.when(cartRepository.getByCustomerId(customerId)).thenReturn(cartEntity);
        Mockito.when(orderItemRepository.findById(orderItemId)).thenReturn(Optional.of(orderItem));
        Mockito.when(orderItemRepository.sumPriceByCartId(cartId)).thenReturn(0.0);

        cartService.updateItemQuantity(customerId, orderItemId, 0);

        Mockito.verify(orderItemRepository).deleteById(orderItemId);
    ...
    @Test
    void removeItem_shouldDeleteItemAndRecalculateTotal() {
        long customerId = 1L;
        long cartId = 3L;
        long orderItemId = 4L;
        CartEntity cartEntity = new CartEntity(cartId, customerId, 21.0);
        ...
        Mockito.when(cartRepository.getByCustomerId(customerId)).thenReturn(cartEntity);
        Mockito.when(orderItemRepository.findById(orderItemId)).thenReturn(Optional.of(orderItem));
        Mockito.when(orderItemRepository.sumPriceByCartId(cartId)).thenReturn(11.0);

        cartService.removeItem(customerId, orderItemId);

        Mockito.verify(orderItemRepository).deleteById(orderItemId);
        Mockito.verify(cartRepository).updateTotalPrice(cartId, 11.0);
    }


    @Test
    void removeItem_whenItemBelongsToAnotherCart_shouldThrowAndNotDelete() {
        long customerId = 1L;
        long cartId = 3L;
        long orderItemId = 4L;
        CartEntity myCart = new CartEntity(cartId, customerId, 0.0);
        // 这个 order item 属于 cart 99，不是当前用户的 cart 3
        OrderItemEntity someoneElsesItem = new OrderItemEntity(orderItemId, 2L, 99L, 10.0, 1);

        Mockito.when(cartRepository.getByCustomerId(customerId)).thenReturn(myCart);
        Mockito.when(orderItemRepository.findById(orderItemId)).thenReturn(Optional.of(someoneElsesItem));

        Assertions.assertThrows(
                ResourceNotFoundException.class,
                () -> cartService.removeItem(customerId, orderItemId));

        Mockito.verify(orderItemRepository, Mockito.never()).deleteById(Mockito.anyLong());
        Mockito.verify(cartRepository, Mockito.never()).updateTotalPrice(Mockito.anyLong(), Mockito.anyDouble());
    }


    @Test
    void removeItem_whenItemDoesNotExist_shouldThrow() {
        long customerId = 1L;
        long orderItemId = 404L;
        CartEntity myCart = new CartEntity(3L, customerId, 0.0);

        Mockito.when(cartRepository.getByCustomerId(customerId)).thenReturn(myCart);
        Mockito.when(orderItemRepository.findById(orderItemId)).thenReturn(Optional.empty());
